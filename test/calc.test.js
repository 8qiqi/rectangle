...
describe('isLegalKey() 测试', function() {
  ...
  describe('合法字符：-', function() {
    it('允许出现在空文本中', function() {
      expect(isLegalKey('-', '', 0)).to.be.ok;
    });

    it('允许出现在合法正数前', function() {
      expect(isLegalKey('-', '3.141', 0)).to.be.ok;
    });

    it('不允许出现在合法正数中间', function() {
      expect(isLegalKey('-', '3.141', 3)).not.to.be.ok;
    });

    it('不允许出现在合法正数末尾', function() {
      expect(isLegalKey('-', '3.141', 5)).not.to.be.ok;
    });

    it('不允许出现在合法负数前', function() {
      expect(isLegalKey('-', '-3.141', 0)).not.to.be.ok;
    });

    it('不允许出现在合法负数中间', function() {
      expect(isLegalKey('-', '-3.141', 4)).not.to.be.ok;
    });

    it('不允许出现在合法负数末尾', function() {
      expect(isLegalKey('-', '-3.141', 6)).not.to.be.ok;
    });
  });
